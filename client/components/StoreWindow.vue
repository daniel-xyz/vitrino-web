...
<script>
  ...
  import { buildImageUrl } from '../js/lib/cloudinary.js'
  ...
  export default {
    ...
    watch: {
      'store.id': function () {
        const self = this;
        let store_window = self.products.store_window;

        store_window.splice(0, store_window.length);

        stores.getStoreWindowProducts(self.store.id, function (error, products) {
    ...
    methods: {
      fillData: function () {
        ...
        // TODO - should be done via vuex as soon as available, as queries are not really reliable for SEO etc.
