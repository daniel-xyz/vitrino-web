...
export const loadAllMarkers = ({ commit, state }, pos) => {
  stores.getStoresInRadius(pos.lat, pos.lng, pos.meters,
    (error, response) => {
      if (error) {
      ...
      response.businesses.forEach((store) => {
        const markerType = state.markerNames[0]; // self.getStoreType(store['product_category']); // eslint-disable-line dot-notation
        ...
        commit(mutations.ADD_MARKER, { type: markerType, marker });
      });
